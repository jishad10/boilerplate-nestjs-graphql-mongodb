 ...
 * UserService
 * ─────────────────────────────────────────────────────────
 * Fully transport-agnostic. File upload methods receive
 * an array of local file paths (written by the resolver
 * from the GraphQL upload stream) rather than Multer objects.
 * This keeps the service clean and reusable.
 */
...
export class UserService {
  ...
  /**
   * @param filePaths - temp file path(s) saved by the resolver from the GQL upload stream
   */
  async createAvatar(userId: string, filePaths: string[]) {
    const user = await this.userModel.findById(userId);
    if (!user) throw new HttpException('User not found', HttpStatus.NOT_FOUND);
    if (!filePaths[0]) throw new HttpException('Profile image is required', HttpStatus.BAD_REQUEST);
  ...
  async updateAvatar(userId: string, filePaths: string[]) {
    const user = await this.userModel.findById(userId);
    if (!user) throw new HttpException('User not found', HttpStatus.NOT_FOUND);
    if (!filePaths[0]) throw new HttpException('Profile image is required', HttpStatus.BAD_REQUEST);
  ...
  async createMultipleAvatars(userId: string, filePaths: string[]) {
    const user = await this.userModel.findById(userId);
    if (!user) throw new HttpException('User not found', HttpStatus.NOT_FOUND);
    if (!filePaths.length) throw new HttpException('Profile images are required', HttpStatus.BAD_REQUEST);

    const urls = await Promise.all(
      filePaths.map((p, i) =>
        this.cloudinaryService
          .upload(p, `${user._id}-${Date.now()}-${i}`, 'user-profile')
          .then((r) => r.url),
      ),
    );
  ...
  async updateMultipleAvatars(userId: string, filePaths: string[]) {
    const user = await this.userModel.findById(userId);
    if (!user) throw new HttpException('User not found', HttpStatus.NOT_FOUND);
    if (!filePaths.length) throw new HttpException('Profile images are required', HttpStatus.BAD_REQUEST);
  ...
  async createPDF(userId: string, filePaths: string[]) {
    const user = await this.userModel.findById(userId);
    if (!user) throw new HttpException('User not found', HttpStatus.NOT_FOUND);
    if (!filePaths[0]) throw new HttpException('PDF file is required', HttpStatus.BAD_REQUEST);
  ...
  async updatePDF(userId: string, filePaths: string[]) {
    const user = await this.userModel.findById(userId);
    if (!user) throw new HttpException('User not found', HttpStatus.NOT_FOUND);
    if (!filePaths[0]) throw new HttpException('PDF file is required', HttpStatus.BAD_REQUEST);
